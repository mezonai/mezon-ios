...
final class VerifyPhoneOTPViewController: BaseViewController {
    ...
    init(context: AccountContext, phoneNumber: String, requestId: String) {
    ...
    override func applyTheme() {
        titleLabel.text = L(L10n.PhoneSetting.verifyPhoneTitle)
        ...
        descriptionLabel.text = "\(L(L10n.PhoneSetting.verifyDescription)) \(phoneNumber)"
        descriptionLabel.textColor = .mezonTextPrimary

        verifyButton.setTitle(L(L10n.PhoneSetting.verifyButton), for: .normal)
    ...
    private func showSuccessAndPop() {
        Toast.success(L(L10n.PhoneSetting.verifySuccess))
    ...
    private func showVerifyFailedToast() {
        Toast.error(L(L10n.PhoneSetting.updateFailed), title: "Error")
    }
}

extension VerifyPhoneOTPViewController: UITextFieldDelegate {
