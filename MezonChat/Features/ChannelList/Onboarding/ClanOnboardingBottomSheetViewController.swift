...
final class ClanOnboardingBottomSheetViewController: UIViewController {
    ...
    private func makeActionRow(_ item: ClanOnboardingActionItem) -> UIView {
        ...
        trailingIcon.isUserInteractionEnabled = false
        if item.isCompleted {
            trailingIcon.image = UIImage(systemName: "checkmark.circle.fill", withConfiguration: UIImage.SymbolConfiguration(pointSize: 22.sf))?
                .withTintColor(UIColor(red: 0.18, green: 0.75, blue: 0.45, alpha: 1), renderingMode: .alwaysOriginal)
        } else {
            trailingIcon.image = UIImage(systemName: "chevron.right", withConfiguration: UIImage.SymbolConfiguration(pointSize: 14.sf, weight: .semibold))?
                .withTintColor(UIColor.theme.text, renderingMode: .alwaysOriginal)
        }

        row.addSubview(iconWrap)
        row.addSubview(titleLabel)
        row.addSubview(trailingIcon)
        ...
        NSLayoutConstraint.activate([
            ...
            titleLabel.trailingAnchor.constraint(lessThanOrEqualTo: trailingIcon.leadingAnchor, constant: -8.sw),

            trailingIcon.trailingAnchor.constraint(equalTo: row.trailingAnchor, constant: -16.sw),
            trailingIcon.centerYAnchor.constraint(equalTo: row.centerYAnchor),
            trailingIcon.widthAnchor.constraint(equalToConstant: 24.swh),
            trailingIcon.heightAnchor.constraint(equalToConstant: 24.swh),
        ])
