...
extension ContainableController {

    func driveAppearanceTransition(appearing: Bool, animated: Bool) {
        (self as? UIViewController)?.beginAppearanceTransition(appearing, animated: animated)
    }

    func finishAppearanceTransition() {
        (self as? UIViewController)?.endAppearanceTransition()
    }
}
